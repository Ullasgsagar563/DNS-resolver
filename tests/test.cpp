...
void testIDNResolution() {
    DNSResolver resolver;
    std::string domain = "xn--d1acpjx3f.xn--p1ai"; // Example IDN domain for "пример.рф"
    DNSResolver::ResolverOptions options;
    auto result = resolver.resolve(domain, options);

    if (result.empty()) {
        throw std::runtime_error("Resolution failed for internationalized domain: " + domain);
    }

    std::cout << "Resolved IP addresses for " << domain << ": ";
    for (const auto& ip : result) {
        std::cout << ip << " ";
    }
    std::cout << std::endl;
...
//     DNSResolver resolver;
//     std::string domain = "xn--d1acpjx3f.xn--p1ai"; // Another IDN domain
//     DNSResolver::ResolverOptions options;
//     auto result = resolver.resolve(domain, options);

//     if (result.empty()) {
//         throw std::runtime_error("Resolution failed for another internationalized domain: " + domain);
//     }
